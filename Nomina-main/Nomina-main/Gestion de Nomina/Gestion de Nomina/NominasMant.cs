    ...
    public partial class NominasMant : Form
    {
        ...
        private void GuardarNomina()
        {
            ...
            try
            {
                ...
                con.OpenConection();
                con.ExecuteQueries("Insert into TablaNominas (Nombre, Apellidos, Cargo, SueldoBruto, HorasExtra, ExtraHours," +
                  " PrecioXHora, BonoTransporte, TotalAsignacion, Seguro, Adelanto, TotalDeduccion, SueldoNeto, Fecha)" +
                " values('" + Nombre + "','" + Apellidos + "' ,'" + Cargo + "','" + SueldoB + "','" + HorasEx + "','" + ExtraHours + "','"+
        ...
        SqlConnection cn = new SqlConnection("Data Source= localhost; Initial Catalog = Nomina; Integrated Security = True");
            ...
            string query = "Select * from TablaNominas";
        ...
        private void SearchBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    con.OpenConection();
                    SqlDataReader dr = con.DataReader("select * from Empleado where ID='" + txtSearch.Text + "'");
                    if (dr.Read())
                    {

                        string nombre = dr.GetString(2);
                        string apellidos = dr.GetString(3);
                        string cargo = dr.GetString(7);
                        string sueldoB = dr.GetString(9);
